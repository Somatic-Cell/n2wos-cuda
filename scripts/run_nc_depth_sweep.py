#!/usr/bin/env python3
"""Run an NC-WoS / NC+2LMC prefix-depth sweep.

This script keeps the cache preset, training schedule, boundary condition,
evaluation points, and NC/2LMC prefix depth coupled. It is intended to test
whether increasing m lowers residual variance without hiding the NC-only bias
comparison behind different prefix depths.
"""
...
def get_run(runs: Dict[str, Any], depth_m: int, *extra: str) -> Dict[str, Any]:
    names = list(extra) + ['nc_2lmc', f'nc_2lmc_m{depth_m}', 'nc_2lmc_m1']
    for name in names:
        value = runs.get(name)
...
def summarize_one(path: pathlib.Path, depth_m: int) -> Dict[str, Any]:
    ...
    return {
        ...
        'total_train_steps': as_int(options.get('label_refreshes')) * as_int(options.get('train_steps_per_refresh')),
        ...
        'nc_wos_mean_steps': as_float(nc.get('mean_steps')),
        ...
        'nc_2lmc_mean_coarse_steps': as_float(two.get('mean_coarse_steps')) if two else float('nan'),
        'nc_2lmc_mean_residual_steps': as_float(two.get('mean_residual_steps')) if two else float('nan'),
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    p.add_argument('--depths', type=parse_int_list, default=parse_int_list('1,2,4'))
    ...
    manifest = {
        'script': 'run_nc_depth_sweep.py',
        'purpose': 'sweep shared prefix depth m for NC-WoS and NC+2LMC',
        'arguments': vars(args).copy(),
        'commands': [],
    }

    rows: List[Dict[str, Any]] = []
    for k, depth_m in enumerate(args.depths):
        seed = args.seed + (k * args.seed_stride if args.seed_stride else 0)
        output_json = out_dir / f'nc_depth_m{depth_m}.json'
        cmd = build_command(args, depth_m, output_json, seed)
        manifest['commands'].append({'depth_m': depth_m, 'seed': seed, 'output': str(output_json), 'command': cmd})
        ...
        if output_json.exists():
            rows.append(summarize_one(output_json, depth_m))
    ...
    if rows:
        best_residual = min(rows, key=lambda r: as_float(r.get('residual_variance_ratio_vs_pure'), float('inf')))
        ...
        print('best residual variance ratio:', best_residual.get('label'), best_residual.get('residual_variance_ratio_vs_pure'))
